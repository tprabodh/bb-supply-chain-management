...
const StockManagerKitchenAssignment = ({ dailyTargets, kitchenManagers, preparedStock }) => {
  ...
  useEffect(() => {
    // Aggregate daily targets passed as prop
    const aggregated = dailyTargets.reduce((acc, target) => {
  ...
  const handleAssign = async (e) => {
    ...
    console.log('forecastId:', forecastId); // Debug log
    console.log('forecastWeek:', forecastWeek); // Debug log
    ...
    kitchenManagers.forEach(km => {
      ...
      if (itemsForThisKitchenManager.length > 0 || fromPreparedStockForThisManager.length > 0) {
        console.log('itemsForThisKitchenManager:', itemsForThisKitchenManager); // Debug log
        console.log('fromPreparedStockForThisManager:', fromPreparedStockForThisManager); // Debug log
...
export default StockManagerKitchenAssignment;
