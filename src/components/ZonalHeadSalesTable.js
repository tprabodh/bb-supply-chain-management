...
const ZonalHeadSalesTable = ({ zonalHead, timePeriod }) => {
  const [isExpanded, setIsExpanded] = useState(false);

  const processSalesData = (teamLeadData, period) => {
    const itemData = {};

    teamLeadData.forEach(tl => {
        tl.salesData.forEach(se => {
            if (se.salesData) {
                for (const dateStr in se.salesData) {
                    ...
                    if (shouldInclude) {
                        const dailySales = se.salesData[dateStr];
  ...
  const processedData = processSalesData(zonalHead.teamLeadData, timePeriod);
  ...
  return (
    <div className="bg-gray-50 rounded-lg shadow-inner p-4">
      <button onClick={() => setIsExpanded(!isExpanded)} className="w-full px-4 py-3 text-left font-semibold text-base sm:text-lg text-[#f56703] hover:text-[#d95702] focus:outline-none transition-colors duration-300">
        {zonalHead.name} - {zonalHead.subrole}
...
export default ZonalHeadSalesTable;
